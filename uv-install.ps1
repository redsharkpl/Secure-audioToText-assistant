...
The installer for uv 0.11.23

.DESCRIPTION

This script detects what platform you're on and fetches an appropriate archive from
https://releases.astral.sh/github/uv/releases/download/0.11.23
...
if ($env:UV_DOWNLOAD_URL) {
  ...
  $ArtifactDownloadUrls = @("$installer_base_url/astral-sh/uv/releases/download/0.11.23")
} elseif ($env:UV_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:UV_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/astral-sh/uv/releases/download/0.11.23")
} else {
  $ArtifactDownloadUrls = @("https://releases.astral.sh/github/uv/releases/download/0.11.23", "https://github.com/astral-sh/uv/releases/download/0.11.23")
}

$auth_token = $env:UV_GITHUB_TOKEN

$receipt = @"
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.31.0"},"source":{"app_name":"uv","name":"uv","owner":"astral-sh","release_type":"github"},"version":"0.11.23"}
"@
